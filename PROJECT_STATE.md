...
**September 25, 2026 — daily/weekly team email (0.10.233–0.10.236).** Reports → Daily email: one SendGrid message, shout-out names on To, everyone else BCC; Mon = weekly recap. **September short calls (answered ≤6s) are at 17.4% month-to-date — over Telnyx's 15% surcharge threshold** (our definition matched their August notice: 15.51% vs 15.48%). "Open Reports blank page" was the preview iframe (sandbox with no scripts) navigating to the app inside itself — fixed in 0.10.236 with `<base target=_blank>` + allow-popups.
**Unrelated, pre-existing, needs someone with JobDiva access:** `[jobdiva] auth: failed` 401 has repeated ~1.2M times since at least 2026-09-08 (the start of the current ace-api log, which is 8.8 GB). JobDiva contact names are not resolving. Check `JOBDIVA_*` credentials in `.env`, and consider pm2-logrotate.
