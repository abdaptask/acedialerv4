...
- **On the host, `npm run build -w apps/web` IS a production deploy.** pm2's `ace-web` serves `apps/web/dist` directly off disk — no copy step, no restart. A build run merely to typecheck publishes the new UI to every web user immediately. If you build it, finish the job: `pm2 reload ace-api` (and `ace-webhooks`/`ace-socket` if they changed) so the backend matches, or you'll ship a frontend calling routes that don't exist yet.
- **Always build the web bundle with `VITE_FORCE_ABSOLUTE_BASE=1` for the self-hosted SPA** (what `deploy.sh` does — `VITE_FORCE_ABSOLUTE_BASE=1 npm run build:web`). Vite's default base is `'./'`, correct for Electron's `file://` load and wrong here: on a nested route the browser resolves `./assets/*.js` against `/settings/` or `/auth/microsoft/`, the SPA fallback returns `index.html` as `text/html`, and the page goes **blank**. Single-segment routes still work, so this hides easily — the casualties are `/settings/:section` and the SSO callback. Verify by checking that `apps/web/dist/index.html` references its bundle **absolutely** (`src="/assets/…"`, not `./assets/…`) and that `curl -D- -o /dev/null http://127.0.0.1:3010/assets/<hashed>.js` returns `application/javascript`. Do NOT test `/settings/assets/<hashed>.js` — an earlier version of this line said to, but that path returns `text/html` even on a **correct** build, because with an absolute base nothing ever requests it and the SPA fallback answers. Reading that as a failure is how someone talks themselves into reverting the fix.
...
- **A scheme added to `HANDLED_SCHEMES` must be accepted in BOTH OS delivery paths.** Windows delivers a protocol launch through argv (`findProtocolUrl`, which reads the list); macOS delivers it through the `open-url` event, which is a *separate* hand-written check. v0.10.218 added `tel:`/`callto:` to the list and to `routeProtocolUrl` but left the `open-url` gate testing `ace-dialer://` alone, so on macOS a `tel:` click brought the app forward and then silently dropped the number — invisible on Windows, and invisible in any test that only exercised argv. Both gates test the same list; keep it that way.
...
- **`tel:` registration and the global hotkey are opt-in and reversible.** Both change behaviour *outside* our app — the first can displace Teams/FaceTime as the system phone handler, the second claims a key combination process-wide. Default off; Settings → Click to dial owns them; `globalShortcut.unregisterAll()` on `will-quit` so a stale binding can't survive the app — **guarded by `app.isReady()`, which is load-bearing.** Losing the single-instance lock calls `app.quit()` at module scope, which emits `will-quit` synchronously *before* ready; touching `globalShortcut` there throws "cannot be used before the app is ready" during module evaluation, so it surfaces as an uncaught-exception dialog rather than a silent exit. That is the ordinary path for every `tel:` link, every deep link, and every launch while we're parked in the tray — i.e. it reads to the user as "crashes on launch". Any `will-quit` / `before-quit` work registered above the lock check needs the same guard.
- **Numbers from highlighted text are validated strictly; numbers from our own database are not.** `src` marks the former — `selection` (extension), `clipboard` (hotkey), or `tel` (link) as of v0.10.221; a marker we don't recognise is treated as untrusted, and only its *absence* means "trusted". Keeping that distinction is what stops click-to-dial from regressing the Teams-card deep links that already work in production.
- **Every capture refusal names the source and quotes the text it read.** The capture paths are invisible — the user selects or copies something, presses a key, and a window appears — so a bare "that's too short to dial" is unfalsifiable and reads as "click to dial is broken". This is not polish: the hotkey reads the CLIPBOARD, so a user who highlights without pressing Ctrl+C gets their previous copy working exactly as designed, and quoting the text is the only way they can ever see that. Main tracks the last clipboard value it forwarded and flags a repeat press so the dialer can explain a field that didn't change.
- **The last-dialed recall must never fire over a field that click-to-dial emptied.** Pressing Call on an empty field recalls `ace_last_dialed` (classic phone behaviour), and a failed capture deliberately empties the field — so the two together silently placed an unrelated old number under a red error, one press away from being dialled. That combination was the actual "click to dial keeps repeating stale numbers" report on 0.10.220. Recall is for a field the *user* left empty; `Dialpad.tsx` suppresses it (and disables the button) while a capture error is showing.
- **An ambiguous capture fails closed — never pick one of two numbers.** `parseSelectedNumber` used to take the first candidate that validated, so among equally long ones the winner was decided by position in the string; on a recruiting screen the leading 10-digit token is as likely to be a candidate id as a phone, and `isValid()` cannot tell them apart. It now evaluates a whole digit-count tier and refuses when the tier holds more than one distinct number, naming both. Tiers, not the whole candidate set — a spaced number and its own trimmed slices resolve to the same E.164, and that case must keep working.
- **The extension may only dial what the chip currently displays.** A chip is a `<span>` injected into someone else's SPA; the framework re-renders rows underneath it, so a dial string parked in `data-ace-dial` at scan time can end up attached to a chip rendering a *different* number. `dial()` compares the live `textContent` against `data-ace-raw` and re-derives (refusing if that's ambiguous) before sending. For the same reason the MutationObserver watches `characterData` and re-derives the whole mutated subtree — melting our chips back to text and rescanning — rather than only looking at `addedNodes`, which can never contain the stale chip. Our own writes are fenced with a `writing` flag plus `takeRecords()`; drop that and the observer feeds itself forever.
- **The browser extension scans pages, so it MUST stay per-domain — never `<all_urls>`.** Auto-detecting numbers requires reading page text; it does not require reading *every* page. The extension ships with no `host_permissions` and no static `content_scripts`, and registers the scanner dynamically (`chrome.scripting.registerContentScripts`) only for domains granted on its options page. Our users work all day in systems full of candidate PII: blanket access would mean a compromise exposes every page they visit, versus only the ATS domains someone deliberately enabled. Same feature, different blast radius — do not "simplify" this to a static `<all_urls>` content script.
- **The page scanner must never touch an editable field.** `<input>`, `<textarea>`, and `contenteditable` are skipped unconditionally — rewriting a field while a recruiter is typing in it would destroy their work, which is far worse than missing a phone number. Same reasoning drives skipping `<script>`/`<style>`/`<a>`, wrapping only in a `<span>` (no layout change), and debouncing the MutationObserver through `requestIdleCallback` so a constantly re-rendering ATS doesn't peg the CPU.
- **Detection is biased toward false negatives, on purpose.** A recruiting page is full of candidate IDs, requisition numbers, and invoice references. A missed number costs one copy-paste; a wrongly underlined ID costs trust in the feature and gets it switched off. Keep `isValid()` (not `isPossible()`), keep the negative-context word list, and add a test when you widen either.
...
- **Graph topology:**
  - Mic → **one shared `GainNode`** → every outgoing destination → all parties hear the user. That gain node is the ONLY self-mute control (see guardrails)
...
- **Self-mute vs participant-mute are different mechanisms.** Self-mute (the main Mute button) sets the mic gain to 0 — the user leaves every leg's mix, everything else keeps flowing. Participant-mute (the per-pill button) disconnects that participant's source node. `toggleMute()` branches on `isConferenceActive()`; `isSelfMuted()` is the read side, and `SipContext` exposes it so the UI can resync across a merge or a drop.
- **Participant-mute path:** disconnect a participant's source node from `ctx.destination` (silences them in user's speakers) AND from every other call's outgoing destination (silences them for the other party). Inbound from that participant is untouched — they still hear everyone.
...
- **Self-mute in conference MUST act on the mic gain node, never `session.mute()`.** JsSIP's mute is `sender.track.enabled = false`, and in conference the sender's track is the **mixed** track (mic + every other participant), not the mic. Muting through it did two wrong things at once: the active leg's participant stopped hearing *everyone* (their relayed audio died with the mix — reported as "mute muted all participants"), while the second leg's sender went untouched, so the user stayed audible to them with the button reading "Unmute". Over-mute one party, under-mute the other. Corollaries: clear any pre-merge `_audioMuted` when the conference starts (JsSIP re-applies it on the next re-INVITE and would disable the mixed track), carry that mute into the gain node instead, and re-apply it to the restored mic tracks on teardown — a fresh `replaceTrack` clone arrives `enabled`, which silently un-mutes the user. Self-mute is graph-owned from the moment Merge commits, not from when `getUserMedia` resolves; the gap is small and a Mute pressed inside it must not fall back to the SIP path. Regression tests: `apps/web/src/services/sipConferenceMute.test.ts`.
...
- **Message length limit is 1600 characters everywhere, enforced in one place.** `MAX_SMS_BODY_CHARS` in `messages/sendMessage.ts` is the single definition; because it's checked inside `sendMessageImmediate`, both the immediate route and the scheduled-message worker inherit it. `POST /messages` and the scheduled-create route also check up front so the user gets a 400 with a sentence rather than a 502. Template bodies and AI rewrite are capped at the same 1600. The web mirror in `Messages.tsx` exists only to disable Send with an explanation — the server is authoritative. Sized from real traffic: p90 body ~337 chars, and nothing in 21,060 messages over 90 days exceeded 1600, so the cap cannot reject a legitimate message.
- **Never put `maxLength` on a compose textarea.** It silently truncates a paste, so the user loses the tail of their message without being told. Disable Send and explain instead.
- **Segments, not characters, are what the carrier bills.** 160 per segment in GSM-7, 153 once concatenated; **70/67 in UCS-2**, which a single emoji, curly quote, or em dash silently triggers — 9.9% of real outbound traffic is non-ASCII and 8.8% carries fixable typographic punctuation. Any surface that composes a message needs the counter from `lib/smsSegments.ts`; the scheduled-send modal shipped without one in 0.10.216 and had to be fixed in 0.10.217.
...
- **Compose drafts are device-local and cleared only when the text goes somewhere real.** `lib/smsDrafts.ts` keys them by thread; the composer restores on mount, flushes **synchronously on unmount** (accepting a call unmounts within milliseconds of the last keystroke, so a debounce alone loses the race the feature exists to win), and clears only after a send resolves — a Telnyx rejection must not destroy the message too. `ThreadDetail` therefore needs `key={active}`: without a remount, a deep link swapping threads leaves one contact's draft in another's box. Wiped on logout alongside the favorites cache, because a draft can hold a candidate's name and rate.
...
- **Activity tab (person view only):** Calls and Texts views of every record — calls with who/when/outcome/talk time, texts as metadata only (when, who, direction, delivery status + carrier reason, billed parts, picture or not; never the body) — plus "by number" / conversation lists. Any record or number opens a timeline of every call and text with that number (`records.tsx`). On a person's report, clicking a call or text KPI lists the records behind it (`DrillSpec.calls` / `.texts` filters), with a By-day toggle. The server builds `callLog` and `textLog` only when the request is scoped to one person.
- **Contact search** (header box; `GET /reports/contact/search` + `/reports/contact`, `apps/api/src/reports/lookup.ts`): digits (4+) or a saved name → matching numbers over the last 12 months → one panel with who on the team contacted it and every call/text/voicemail, newest first. Admins search everyone; others only their own history (server-enforced). The digits-only last-10 compare is raw SQL for the same reason as §30.3.
- **Follow-ups tab** (`GET /reports/follow-ups`): live, last 14 days, independent of the range picker. Open items = missed calls with no call/text back since the LAST attempt, text threads whose last message is theirs (a STOP is never an open item), voicemails with no callback. A missed call that left a voicemail is one item, not two. `listenedAt` is deliberately not used — listening from Teams/email may not set it.
- **Insights tab (admin only):** best time to reach (outbound calls connected ≥30s by weekday×hour, cells under 25 calls blank), candidates contacted by 2+ people, opt-outs and texts sent after them, numbers failing as not-found 2+ times, biggest movers in calls out (baseline ≥50). Opt-out = an inbound text that is ONLY a CTIA keyword (STOP/STOPALL/UNSUBSCRIBE/CANCEL/END/QUIT/OPT OUT/REVOKE); START/UNSTOP re-opts in. "yes" is NOT an opt-in — it's the most common candidate reply.
- **End reasons** (`endReason()` in `canonicalCalls.ts`): one plain-language reason per call, from the most specific evidence — app packet counts (`rxPackets` 0 on a connected call ≥5s = "no audio", the one-way-audio case), confirmed-drop causes, the app's JsSIP originator, then Telnyx `hangup_source` (named per call: on outbound we are the caller, on inbound they are), and `sip_hangup_cause` when Telnyx sends one. Text failures go through `textFailureReason()` (`textReasons.ts`), mapping Telnyx codes to actionable sentences. The webhooks service stores `sip_hangup_cause` and `call_quality_stats` (as `carrierStats`) on every hang-up; `carrier_stats->'inbound'->>'packet_count'` is read as `carrierRxPackets` but nothing depends on it until real payloads confirm the shape.
- **Unique numbers dialled** (`uniqueDialled`, `uniqueConnected`): distinct other-party numbers per person; team total is a union.
- **Daily email tab (admin, team view)** (`apps/api/src/reports/digest.ts`, `GET /reports/digest`, `POST /reports/digest/send`, `PUT /reports/digest/schedule`): Tue–Fri covers the previous working day; Monday is a weekly recap of the previous Mon–Fri (adds a "Most improved" box, baseline ≥50 calls; dedup key `from..to`). Includes a short-calls box: answered calls ≤6s as a share of answered calls, month to date, against Telnyx's 15% surcharge threshold (`CARRIER_SHORT_SEC`/`CARRIER_SHORT_LIMIT` in compute.ts; this definition gave 15.51% for Aug 2026 vs Telnyx's notice of 15.48%), team totals only; and up to four best practices (`bestPractices()`): two of the data-triggered tips that apply (rotated by date, so a problem that persists doesn't lead every email), the best hour to call next, and one general practice from a ~20-item rotation. Recipients and the To/BCC split are computed at send time from that period's shout-outs. ONE SendGrid message per send, built from `computeReport()` (same numbers as the page): the people named in the shout-outs on To, every other recipient on BCC (to/bcc disjoint; no shout-outs → To is our own sending address). Team-level content only — the leader rejected per-person sections as "sending everyone my logs". Recipients = active users with a call or text in the last 28 days. Shout-outs are three boxes (emoji icon: the only icon Outlook reliably renders), top three per positive measure, never a bottom list. A failed automatic send releases the day's claim so the next tick retries. Sending to everyone requires the client's `expectedCount` to match and refuses a second send for the same day unless `force`. The weekday scheduler (`startDigestScheduler`, 5-min tick) claims the day with a compare-and-set on the `daily_digest` SystemSetting before sending; every send is audited as `reports.digest_sent` (actor null when automatic).
- **Spend is admin-only:** for non-admins `stripCost()` removes `cost` and every cost/segment field from the payload; the Cost tab is `adminOnly`.
