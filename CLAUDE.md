...
- **Message length limits, as actually enforced:** template bodies are capped at **1600 chars** (zod, both admin and personal routes, plus `maxLength` on the editor textarea); AI rewrite input/output is capped at 1600 (`REWRITE_MAX_CHARS`). `POST /messages` itself has **no length cap** — it forwards whatever it's given to Telnyx, which segments it. The composer's counter ([[18-messaging]] `smsSegments.ts`) is the user-facing guide: 160 chars per segment in GSM-7, 153 when concatenated; **70/67 in UCS-2**, which any emoji, curly quote, or em dash silently triggers. If a hard send-side cap is ever wanted, 1600 (10 GSM-7 segments) is the number to match the rest of the system.
