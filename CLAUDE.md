...
- **Daily email tab (admin, team view)** (`apps/api/src/reports/digest.ts`, `GET /reports/digest`, `POST /reports/digest/send`, `PUT /reports/digest/schedule`): per-recipient SendGrid email built from `computeReport()` (same numbers as the page) + `computeFollowUps()`. Recipients = active users with a call or text in the last 28 days. Personal section and follow-ups only in that person's copy; shout-outs are top-three, positive measures only. Sending to everyone requires the client's `expectedCount` to match and refuses a second send for the same day unless `force`. The weekday scheduler (`startDigestScheduler`, 5-min tick) claims the day with a compare-and-set on the `daily_digest` SystemSetting before sending; every send is audited as `reports.digest_sent` (actor null when automatic).
