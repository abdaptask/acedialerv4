...
export const WHATS_NEW: ReleaseEntry[] = [
  {
    ...
    changes: [
      { type: 'new', text: 'On every user row in Settings → Users, the kebab menu now has "Refresh from Pulse". One click opens a modal that re-pulls their last 30 days of SMS history from Pulse into ACE. If you also paste their Pulse password, calls get refreshed too. No command-line, no IDs to look up — just point and click.' },
      { type: 'new', text: 'Settings → Users → "Bulk-refresh SMS" runs the same SMS refresh across every migrated user in one shot. Per-user results table shows the new-message count for each. Useful as a nightly cleanup pass after recent migrations.' },
      { type: 'improved', text: 'Bulk refresh is SMS-only by design. Calls can\'t be bulk-refreshed because Pulse scopes call logs to each user\'s own JWT and ACE doesn\'t store user passwords. For a specific user\'s call refresh, use the per-user "Refresh from Pulse" button on their row and provide their Pulse password.' },
    ],
  },
  {
    version: '0.10.37',
    ...
    changes: [
      { type: 'new', text: 'Settings → Users → "Migrate from Pulse". Enter the user\'s Pulse email and password, and ACE does everything else in 30-60 seconds: creates their ACE account, moves their phone number from Pulse to ACE in Telnyx, configures the messaging profile, sends a welcome email, and imports their last 30 days of calls + SMS. Step-by-step results shown in the modal so you can see exactly what happened.' },
      { type: 'new', text: 'Brand-new admin endpoint POST /admin/users/migrate-from-pulse for the same flow over the API. Useful for scripting bulk migrations.' },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: 'improved', text: 'Admins can re-run a user\'s backfill at any time by providing that user\'s Pulse email + password to the backfill endpoint. The password is used once and never stored; audit log records that a backfill ran but not the credentials.' },
      { type: 'fixed', text: 'Pulse marks call records as "incoming"/"outgoing" rather than "inbound"/"outbound". The import now recognises both — earlier some incoming calls were incorrectly tagged as outbound.' },
    ],
  },
  {
    version: '0.10.34',
