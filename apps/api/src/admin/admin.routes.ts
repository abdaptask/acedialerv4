...
export async function adminRoutes(app: FastifyInstance) {
  ...
  const BackfillSchema = z.object({
    ...
    // call-log path; SMS uses the service account JWT. Password is used
    // once and never persisted. Email defaults to the ACE user's email
    // when omitted.
  ...
  app.post<{ Params: { id: string; didId: string } }>(
    ...
    async (request, reply) => {
      ...
      // v0.10.36 — Audit log captures whether a Pulse password was used,
  ...
  // request.
  //
  // Failure model (deliberate, see v0.10.37 design discussion): if any
  // step after the User row is created fails, we LEAVE the user in place
  // and return ok=false with `steps[]` telling the admin which step
  // failed. Rationale: Telnyx and email hiccups are transient; rolling
  // back the user would cause duplicate-user errors on retry. Admin can
  // finish the failed step manually via the existing per-DID endpoints.
  //
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      //    Same template-clone path invite-new uses.
      ...
      if (!rebind.ok) {
        ...
        // Note: connection orphaned. Will need cleanup. Leave for admin per failure-model.
      ...
      // 7) Bind the DID to ACE's messaging profile (SMS routing)
      if (config.telnyxMessagingProfileId) {
        const bind = await telnyx.assignNumberMessagingProfile(
      ...
      if (callerIdRes.ok) {
        step(`set Caller ID Override = ${e164}`, true);
      } else {
        step(`set Caller ID Override = ${e164}`, false, JSON.stringify(callerIdRes.error));
      }
      ...
      // 10) UserDid row + set default outbound
      const linked = await ensureUserDid({
      ...
      // Audit (never include the password value)
  ...
  //
  // v0.10.38b — Per-user "Refresh from Pulse" button. The UI just sends
  // the target user id and (optionally) the user's Pulse password. The
  // backend resolves everything else:
  //   - Pulse user_id  → latest entry in audit_logs (user.migrated_from_pulse
  //                       or user_did.backfill_rerun)
  //   - DID            → user's default UserDid
  //   - Email          → the user's ACE email (same as Pulse)
  //
  // If password is provided, calls + SMS are refreshed. If not, only SMS
  // (per the existing pulseApi.ts design).
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      // Find the user + their default DID
      ...
      if (!user) {
        return reply.code(404).send({ ok: false, error: 'User not found in ACE' });
      }
      if (!user.isActive) {
        return reply.code(409).send({ ok: false, error: 'User is deactivated' });
      }
      ...
      if (pulseUserId === null) {
        return reply.code(409).send({
          ok: false,
          error: 'No Pulse user_id on record for this user. They may not have been migrated from Pulse, or the audit log entry was lost. To set one up, use the per-DID backfill endpoint with pulseUserIdOverride.',
        });
      }
      ...
      const result = await backfillMigratedDidHistory(
        {
          ...
          pulseUserPassword,    // undefined skips the calls REST path
        },
        (obj, msg) => request.log.info(obj, msg),
      );
  ...
  //
  // v0.10.38 — Iterates every ACE user with a known Pulse origin and
  // re-runs the MySQL SMS backfill for them. Use case: reconciliation
  // when Pulse keeps logging new SMS for a day or two after a user
  // migrates to ACE, and you want those late-arriving messages pulled
  // into ACE without admin per-user clicking.
  //
  // SMS-only, not calls — calls require the user's own Pulse JWT (Pulse's
  // /telnyx/getCallLogs has no admin override). We don't store Pulse
  // passwords, so bulk-refreshing calls isn't possible without prompting
  // for each user's password individually.
  //
  // How we identify migrated users: query the audit log for entries that
  // record a Pulse user_id — either the migrate-from-pulse wizard entries
  // ('user.migrated_from_pulse') or the manual backfill entries
  // ('user_did.backfill_rerun' with pulseUserIdOverride in metadata).
  // Group by targetUserId, take the most recent pulseUserId for each.
  //
  // Sequential processing with a small inter-user delay (200ms) to stay
  // polite to Pulse MySQL. Capped at 100 users per invocation. For larger
  // tenants admin can re-run; idempotent via skipDuplicates.
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      // 1. Find unique migrated users via audit log. Look at the two action
      //    types that record a Pulse user_id in metadata. Newest entry per
      //    user wins (in case admin re-ran a backfill with a different id).
      ...
      for (const row of auditRows) {
        ...
        if (userPulseMap.has(row.targetUserId)) continue;   // newest-first; first seen wins
      ...
      // 2. For each user, fetch their default UserDid + email, then run the
      //    backfill with pulseUserIdOverride (no password — calls will skip).
      ...
      for (const [userId, pulseUserId] of entries) {
        ...
        // Backfill — pulseUserPassword intentionally omitted so the calls
        // REST path no-ops; only MySQL SMS runs.
        ...
        // Be polite to Pulse MySQL — 200ms between users
